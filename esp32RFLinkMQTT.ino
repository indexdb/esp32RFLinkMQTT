...
AsyncWebServer httpserver(80);  // Create a webserver object that listens for HTTP request on port 80
...
void loadEEPROM() {
  EEPROM.begin(4096);
  DEBUG_PRINT("EEPROM size: ");
  DEBUG_PRINTLN(EEPROM.length());
  DEBUG_PRINT("EEPROM configuration max size: ");
  DEBUG_PRINTLN(eepromConfigMaxSize);
...
void showEEPROM() {
  ...
  DEBUG_PRINTLN(" - WiFi");
  DEBUG_PRINT(" \t| ssid: ");
  DEBUG_PRINTLN(eepromConfig.ssid);
  DEBUG_PRINT(" \t| password: ");
  DEBUG_PRINTLN(eepromConfig.psk);
  DEBUG_PRINT(" \t| hostname: ");
  DEBUG_PRINTLN(eepromConfig.hostname);
  DEBUG_PRINTLN(" - MQTT");
  DEBUG_PRINT(" \t| server: ");
  DEBUG_PRINTLN(eepromConfig.mqtt.server);
  DEBUG_PRINT(" \t| port: ");
  DEBUG_PRINTLN(eepromConfig.mqtt.port);
  DEBUG_PRINT(" \t| user: ");
  DEBUG_PRINTLN(eepromConfig.mqtt.user);
  DEBUG_PRINT(" \t| password: ");
  DEBUG_PRINTLN(eepromConfig.mqtt.password);
  DEBUG_PRINTLN(" - MEGA");
  DEBUG_PRINT(" \t| mega reset pin: ");
  DEBUG_PRINTLN(eepromConfig.mega_reset_pin);
  DEBUG_PRINT(" \t| auto reset interval: ");
  DEBUG_PRINTLN(eepromConfig.resetMegaInterval);
  DEBUG_PRINT(" - ID filtering: ");
  DEBUG_PRINTLN((eepromConfig.id_filtering) ? "enabled" : "disabled");
  for (int i = 0; i < FILTERED_ID_SIZE; i++) {
    DEBUG_PRINT(" \t| ");
    DEBUG_PRINT(eepromConfig.filtered_id[i].id);
    DEBUG_PRINT(" \t\t- ");
    DEBUG_PRINT(eepromConfig.filtered_id[i].id_applied);
    DEBUG_PRINT(" \t\t- ");
    DEBUG_PRINT(eepromConfig.filtered_id[i].publish_interval);
    DEBUG_PRINT(" \t\t- ");
    DEBUG_PRINT(eepromConfig.filtered_id[i].description);
    DEBUG_PRINTLN();
  }
  DEBUG_PRINT(" - Settings locked ");
  DEBUG_PRINTLN(eepromConfig.settings_locked);
  DEBUG_PRINT(" - Version ");
  DEBUG_PRINTLN(eepromConfig.version);
};
...
void checkEEPROM() {
  ...
  DEBUG_PRINT("Config version in EEPROM:    ")
  DEBUG_PRINTLN(eepromConfig.version);
  DEBUG_PRINT("Config version in config.h:  ")
...
boolean mqttConnect() {
  ...
  DEBUG_PRINT(F("MQTT connection attempts: "));
  DEBUG_PRINTLN(mqttConnectionAttempts);
...
void ConfigHTTPserver() {
  httpserver.on("/esp.css", HTTP_GET, [](AsyncWebServerRequest *request) {
    //request->sendHeader("Access-Control-Max-Age", "86400");
    String cssMessage = FPSTR(cssDatasheet);
    request->send(200, "text/css", cssMessage);
  });
  ...
  httpserver.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    // URL arguments
    int page = 0;
    DEBUG_PRINTLN("Html page requested: /00");
    if (request->hasArg("page")) {
      DEBUG_PRINTLN("Html page requested: /01");
      String arg_page = request->getParam("page", true)->value();
      DEBUG_PRINTLN("Html page requested: /02");
      if (arg_page.length() > 0) {
        page = max((int)arg_page.toInt(), 0);
      };
    }

    DEBUG_PRINTLN("Html page requested: /1");
    String htmlMessage = "";
    // Page start
  ...
  httpserver.on("/livedata", [](AsyncWebServerRequest *request) {  //
  ...
  httpserver.on("/reboot", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    request->send(200, "text/html", "Rebooting ESP...");
    delay(500);
    ESP.restart();
    //ESP.reset();
  });

  httpserver.on("/reset-mega", [](AsyncWebServerRequest *request) {  // Reset MEGA
    DEBUG_PRINTLN("Html page requested: /reset-mega");

    DEBUG_PRINTLN("Resetting Mega...");
    request->send(200, "text/html", "Resetting Mega...");
  ...
  httpserver.on("/send", [](AsyncWebServerRequest *request) {  // Handle inputs from web interface
    DEBUG_PRINTLN("Html page requested: /send");

    if (request->args() > 0) {
      for (uint8_t i = 0; i < request->args(); i++) {
        if (request->argName(i) == "command") {   // Send command to RFLInk from web interface, check it comes from command input in html form
          String text_command = request->arg(i);  // Get command send
          ...
          //rflinkSerialTX.print(request->getParam(i));
          rflinkSerialTX.print(F("\r\n"));
          DEBUG_PRINTLN();
          DEBUG_PRINTLN(F("=== Web command ==="));
          DEBUG_PRINT(F("message = "));
          DEBUG_WRITE(buf, sizeof(buf));
          DEBUG_PRINT(request->arg(i));
          DEBUG_PRINTLN(F("\r\n"));
        }
      }
    }
  ...
  httpserver.on("/enable-debug", [](AsyncWebServerRequest *request) {
    DEBUG_PRINTLN("Enabling MQTT debug...");
    MQTT_DEBUG = 1;
    //request->addHeader("Location","/system");
    //request->send(303);
    request->redirect("/system");
  });
  httpserver.on("/disable-debug", [](AsyncWebServerRequest *request) {
  ...
  httpserver.on("/enable-id_filtering", [](AsyncWebServerRequest *request) {
    DEBUG_PRINTLN("Enabling ID filtering...");
    eepromConfig.id_filtering = 1;
    saveEEPROM();
    request->redirect("/system");
    //request->sendHeader("Location","/system");
    //request->send(303);
  });

  httpserver.on("/disable-id_filtering", [](AsyncWebServerRequest *request) {
  ...
  httpserver.on("/lock-settings", [](AsyncWebServerRequest *request) {
    DEBUG_PRINTLN("Enabling ID filtering...");
  ...
  httpserver.on("/unlock-settings", [](AsyncWebServerRequest *request) {
    DEBUG_PRINTLN("Disabling ID filtering...");
    eepromConfig.settings_locked = 0;
    saveEEPROM();
    request->redirect("/system");
  ...
  httpserver.on("/data.txt", [](AsyncWebServerRequest *request) {  // Used to deliver raw data received (BUFFER) and mqtt data published (MQTT_TOPIC and JSON)
    request->send(200, "text/html", "<td>" + String(BUFFER) + "</td><td>" + String(MQTT_TOPIC) + "</td><td>" + String(JSON) + "</td>\r\n");
  });

  httpserver.on("/wifi-scan", [](AsyncWebServerRequest *request) {
    DEBUG_PRINTLN("Html page requested: /wifi-scan");
    int numberOfNetworks = WiFi.scanNetworks();
    DEBUG_PRINTLN("Html page requested: /wifi-scan-0");
    String htmlMessage = "";
    htmlMessage += "<table>\r\n";
    DEBUG_PRINTLN("Html page requested: /wifi-scan-1");
    for (int i = 0; i < numberOfNetworks; i++) {
      htmlMessage += "<tr><td>" + String(i + 1) + " - " + String(WiFi.SSID(i)) + " </td><td> " + String(WiFi.RSSI(i)) + " dBm </td><tr>\r\n";
      DEBUG_PRINTLN(htmlMessage);
      DEBUG_PRINTLN("Html page requested: /2");
    }
    htmlMessage += "</table>\r\n";
    DEBUG_PRINTLN("Html page requested: /3");
    request->send(200, "text/html", htmlMessage);
  });

  httpserver.on("/system", [](AsyncWebServerRequest *request) {
    ...
    // ID filtering
    if (!eepromConfig.settings_locked) {
      htmlMessage += "<tr><td>ID Devices</td><td>";
    ...
    htmlMessage += "<tr><td style='min-width:150px;'><a class='button link' href='/reboot' onclick = \"fetchAndNotify('/reboot');return false;\">&#128268;&nbsp;Reboot&nbsp;ESP</a></td><td>Restart esp32RFLinkMQTT</td></tr>\r\n";
  ...
  httpserver.on("/update-settings", [](AsyncWebServerRequest *request) {  // Used to change settings in EEPROM
    DEBUG_PRINTLN("Html page requested: /update-settings");
    // URL arguments
#ifdef ENABLE_WIFI_SETTINGS_ONLINE_CHANGE
    if (request->hasArg("save_wifi")) {
      strncpy(eepromConfig.ssid, request->getParam("ssid", true)->value().c_str(), CFG_SSID_SIZE);
      strncpy(eepromConfig.psk, request->getParam("psk", true)->value().c_str(), CFG_PSK_SIZE);
      strncpy(eepromConfig.hostname, request->getParam("hostname", true)->value().c_str(), CFG_HOSTNAME_SIZE);
      saveEEPROM();
...
#ifdef ENABLE_MQTT_SETTINGS_ONLINE_CHANGE
    DEBUG_PRINTLN("Html page requested: /update-settings");
    if (request->hasArg("save_mqtt")) {
      int itemp;
      strncpy(eepromConfig.mqtt.server, request->getParam("mqtt_server", true)->value().c_str(), CFG_MQTT_SERVER_SIZE);
      itemp = request->arg("mqtt_port").toInt();
      eepromConfig.mqtt.port = (itemp > 0 && itemp <= 65535) ? itemp : MQTT_PORT;
      strncpy(eepromConfig.mqtt.user, request->getParam("mqtt_user", true)->value().c_str(), CFG_MQTT_USER_SIZE);
      strncpy(eepromConfig.mqtt.password, request->getParam("mqtt_password", true)->value().c_str(), CFG_MQTT_PASSWORD_SIZE);
      saveEEPROM();
      DEBUG_PRINTLN("MQTT settings updated.");
      DEBUG_PRINTLN("Html page requested: /update-settings-2");
    }
#endif

    // Pin to reset MEGA
    if (request->hasArg("save_mega_reset_pin")) {
      int itemp = request->getParam("mega_reset_pin", true)->value().toInt();
      eepromConfig.mega_reset_pin = (itemp >= -1 && itemp <= 16) ? itemp : -1;
      saveEEPROM();
      DEBUG_PRINTLN("MEGA reset pin updated.");
    }
    DEBUG_PRINTLN("Html page requested: /update-settings-3");
    // Auto reset MEGA interval
    if (request->hasArg("save_auto_reset_mega_interval")) {
      eepromConfig.resetMegaInterval = request->getParam("resetMegaInterval", true)->value().toDouble() * 1000;
      saveEEPROM();
      DEBUG_PRINTLN("Auto reset MEGA interval updated.");
    }
    DEBUG_PRINTLN("Html page requested: /update-settings-4");
    ...
    //request->addHeader("Location","/system");
    //request->send(303);
    DEBUG_PRINTLN("Html page requested: /update-settings-5");
  });

  httpserver.on("/idfiltering", [](AsyncWebServerRequest *request) {  // Used to change filtered_IDs configuration in EEPROM
    DEBUG_PRINTLN("Html page requested: /idfiltering");

    // URL arguments
   int page = 0;
    DEBUG_PRINTLN("Html page requested: /idfiltering 00");
    if (request->hasArg("page")) {
      DEBUG_PRINTLN("Html page requested: /idfiltering 01");
      String arg_page = request->getParam("page", true)->value();
      DEBUG_PRINTLN("Html page requested: /idfiltering 02");
      if (arg_page.length() > 0) {
        page = max((int)arg_page.toInt(), 0);
      };
    }
    if (request->hasArg("save_configuration")) {
      //for (int i = 0+(16*page); i < min(filtered_id_number,16*(page+1)); i++){

      for (int i = 0; i < filtered_id_number; i++) {
        if (request->hasArg(("id[" + String(i) + "]").c_str())) {
          String arg_id = request->getParam(("id[" + String(i) + "]").c_str(), true)->value();
          String arg_id_applied = request->getParam(("id_a[" + String(i) + "]").c_str(), true)->value();
          String arg_description = request->getParam(("id[" + String(i) + "]").c_str(), true)->value();
          String arg_publish_interval = request->getParam(("pi[" + String(i) + "]").c_str(), true)->value();
  ...
  httpserver.on("/idfiltering_config_h", [](AsyncWebServerRequest *request) {  // Show code to update config.h with current ID filtering configuration
  ...
  httpserver.on("/idfiltering_restore_curl", [](AsyncWebServerRequest *request) {  // Command to restore ID filtering configuration with curl
  ...
  httpserver.on("/idfiltering_restore_url", [](AsyncWebServerRequest *request) {  // Url to restore ID filtering configuration from browser
  ...
  httpserver.on("/configuration.json", [](AsyncWebServerRequest *request) {  // File to save configuration in json
    ...
    AsyncWebServerResponse *response = request->beginResponse(200, "text/html", htmlMessage);
  ...
  httpserver.on("/save-eeprom", [](AsyncWebServerRequest *request) {  // Saving EEPROM + reboot
    DEBUG_PRINTLN("Html page requested: /save-eeprom");
    DEBUG_PRINTLN("Saving EEPROM");
    saveEEPROM();
    DEBUG_PRINTLN("Rebooting device...");
    request->send(200, "text/html", "EEPROM saved, rebooting...");
    delay(500);
    ESP.restart();
    //ESP.reset();
  });

  httpserver.on("/erase-eeprom", [](AsyncWebServerRequest *request) {  // Erasing EEPROM + reboot
    DEBUG_PRINTLN("Html page requested: /erase-eeprom");
    DEBUG_PRINTLN("Erasing EEPROM configuration");
    DEBUG_PRINT("EEPROM config max size: ");
    DEBUG_PRINTLN(eepromConfigMaxSize);
    EEPROM.begin(4096);
    ...
    EEPROM.end();
    DEBUG_PRINTLN("Rebooting device...");
    request->send(200, "text/html", "EEPROM configuration erased, rebooting...");
    delay(500);
    ESP.restart();
    //ESP.reset();
  });
...
#ifdef ENABLE_SERIAL_DEBUG
  httpserver.on("/read-eeprom", [](AsyncWebServerRequest *request) {  // Erasing EEPROM + reboot
...
#define listBytes 4096
    ...
    delay(100);
    EEPROM.end();
    request->send(200, "text/html", "EEPROM read to serial");
    //request->sendHeader("Location","/system");
    //request->send(303);
  });
#endif
...
void publishUptimeAndRssi() {
  ...
  DEBUG_PRINT("Uptime : ");
  DEBUG_PRINTLN(time_string_exp(millis()));
...
// Setup WiFi
  ...
  DEBUG_PRINT("Size of eepromConfig: ");
  DEBUG_PRINTLN(sizeof(eepromConfig));
  DEBUG_PRINT("Size of filtered_IDs: ");
  DEBUG_PRINTLN(sizeof(filtered_IDs));
  DEBUG_PRINT("Size of matrix: ");
  DEBUG_PRINTLN(sizeof(matrix));
  ...
  if (!MQTTClient.connected()) {
    ...
    if (now - lastMqttConnect > MQTT_CHECK_INTERVAL) {
      ...
      if (mqttConnectionAttempts != 0) {
        DEBUG_PRINT(" - retrying after ");
        DEBUG_PRINT(MQTT_CHECK_INTERVAL / 1000);
        DEBUG_PRINT(" s");
      }
      DEBUG_PRINTLN();
      lastMqttConnect = now;
      ...
      for (int i = 0; i < (filtered_id_number); i++) {
        ...
        if (strcmp(MQTT_ID, eepromConfig.filtered_id[i].id) == 0) {
          ...
          if ((strncmp(matrix[i].json, JSON, sizeof(matrix[i].json) - 2) != 0) || (!matrix[i].last_published)) {
            ...
            DEBUG_PRINT(" => data changed or never published => published on ");
